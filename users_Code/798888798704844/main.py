...
happend = []
all_Things = []
def control_List_Reference(value):
    list_Value = list(value)
    open = False
    string = False
    return_Var = False
    for i in range(0, len(list_Value)):
        string = control_String(list_Value[i],string)
        if value[i] == "[" and not string:
            open = True
        elif value[i] == "]" and not string:
            open = False
        if not open and value[i] != "[" and value[i] != "]" and value[i] != "":
            return_Var = True
    return return_Var
...
def get_List(value_Name, value):
    name, all_Index = (list_Control(value_Name))
    this_List = []
    index = 0
    end_List = []
    for i in range(0,len(values_Of_Variables)):
        if values_Of_Variables[i][0] == name:
            this_List = values_Of_Variables[i][1][0]
            index = i
    if len(this_List):
        all_List = [this_List]
        for i in all_Index:
            try:
                this_List = dir_To_This_List(this_List,i)
        ...
        if len(data[i].split("(")) > 1:
            ...
            for k in this_ListX:
                if k == ")":
                    ...
                    if not last[0]:
                        if type(open_List[index][2]) == "<class = 'str'>":
            ...
            if len(values_Of_Variables[index_List[i]][0].split("[")) > 1:
                values_Of_Variables[index_List[i]][1] =  get_List(values_Of_Variables[index_List[i]][0], value)
            else:
...
def dir_To_This_List(list, index):
    return list[index]

def list_Control(name):
    list_Name = list(name)
    index = ""
    open = False
    new_Name = ""
    ...
    for i in list_Name:
        ...
        if open and i != "[":
            ...
            new_Name += i
    return [new_Name, all_Index]

def search_In_All_Var(name = None, value = None):
    end = False
    if name:
        for i in range(0, len(values_Of_Variables)):
            if values_Of_Variables[i][0] == name:
                end = values_Of_Variables[i][1]
            else:
                pass
    if value:
        for i in range(0, len(values_Of_Variables)):
            if values_Of_Variables[i][1] == value:
                end = values_Of_Variables[i][0]
    return end
...
def declarate(this_List):
    ...
    if a[0] == -2:
        ...
        if control_List_Reference(value[0]):
            this_Name, all_Index = (list_Control(value[0]))
            value_List = search_In_All_Var(name = this_Name)[0]
            if value_List:
                for i in range(0, len(all_Index)):
                    try:
                        value_List = dir_To_This_List(value_List, all_Index[i])
                    except:
                        break
                else:
                    value = value_List
